...
* 1. `D5_Baseline_Est_Start_water` (for decile 5) = 25% of summer rainfall 
...
* 4. `estimated_crop_water_use` (mm) = `D5_Baseline_Est_Start_water` + `rainfall_to_date` + `rainfall_to_end_GS`
...
* 1. `Frontier_Est_Start_water` (for decile 5) = 30% of summer rainfall OR 25% of summer rainfall + 6.
...
* 3. `rainfall_to_end_GS` = *not sure what this is, perhaps its the amount of rainfall in GS - rainfall to defined date*
* 4. `estimated_crop_water_use` (mm) = `Frontier_Est_Start_water` + `rainfall_to_date` + `rainfall_to_end_GS`
...
plot1 <-b_f_forecast_yld %>%
  ...
  ggplot(aes(x = GS_deciles_names , y = Yield_forecast, group=method) )+
