...
Day_defined_date4 <- 15 
Month_defined_date4 <- 6 

Day_defined_date5 <- 1 
Month_defined_date5 <- 7

Day_defined_date6 <- 15 
Month_defined_date6 <- 7 

Day_defined_date7 <- 1 
Month_defined_date7 <- 8 

Day_defined_date8 <- 1 
Month_defined_date8 <- 9 

Day_defined_date9 <- 1 
Month_defined_date9 <- 10
...
date9 <- paste0(Day_defined_date9, "-", Month_defined_date9)
...
df <- df %>% mutate(
  start_end_GS_date8 = case_when(
    month == Month_defined_date8 & day_of_month == Day_defined_date8 ~ "start_gs",
    month == Month_end_GS_rainfall & day_of_month == Day_end_GS_rainfall ~ "end_gs",
    TRUE ~ NA))

df <- df %>% mutate(
  start_end_GS_date9 = case_when(
    month == Month_defined_date9 & day_of_month == Day_defined_date9 ~ "start_gs",
    month == Month_end_GS_rainfall & day_of_month == Day_end_GS_rainfall ~ "end_gs",
    TRUE ~ NA))
...
df <- df %>% 
  fill(start_end_GS_date8) %>%
  mutate(  season_GS8 = case_when(
    start_end_GS_date8 == "start_gs" ~ "gs",
    TRUE ~ "other"))

df <- df %>% 
  fill(start_end_GS_date9) %>%
  mutate(  season_GS9 = case_when(
    start_end_GS_date9 == "start_gs" ~ "gs",
    TRUE ~ "other"))
...
gs_rain8 <- df %>%
  group_by(year, season_GS8) %>%
  summarise(sum_rain_season = sum(Rain, na.rm = TRUE)) %>%
  filter(season_GS8 == "gs")
gs_rain9 <- df %>%
  group_by(year, season_GS9) %>%
  summarise(sum_rain_season = sum(Rain, na.rm = TRUE)) %>%
  filter(season_GS9 == "gs")
...
GS_decile_table7 <- data.frame(
  ...
  select(GS_deciles_names, GS_decile_min_rain, GS_decile_max_rain, defined_date)
#add in the min value to table
GS_decile_table7[1,2] <-  min(gs_rain7$sum_rain_season)

#date8
GS_decile_max_rain8 <- quantile(gs_rain8$sum_rain_season, c(.1, .2, .3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0))
GS_deciles_names8 <- c("decile_1", "decile_2", "decile_3", "decile_4", "decile_5", "decile_6", "decile_7", "decile_8", "decile_9", "decile_10")

GS_decile_table8 <- data.frame(
  GS_decile_max_rain = GS_decile_max_rain8,
  GS_deciles_names = GS_deciles_names8,
  row.names = NULL
) %>%
  mutate(GS_decile_min_rain = lag(GS_decile_max_rain8 + 0.1)) %>%
  mutate(defined_date = paste0("from ", date8, "to 14-10")) %>%
  select(GS_deciles_names, GS_decile_min_rain, GS_decile_max_rain, defined_date)
#add in the min value to table
GS_decile_table8[1,2] <-  min(gs_rain8$sum_rain_season)


#date9
GS_decile_max_rain9 <- quantile(gs_rain9$sum_rain_season, c(.1, .2, .3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0))
GS_deciles_names9 <- c("decile_1", "decile_2", "decile_3", "decile_4", "decile_5", "decile_6", "decile_7", "decile_8", "decile_9", "decile_10")

GS_decile_table9 <- data.frame(
  GS_decile_max_rain = GS_decile_max_rain9,
  GS_deciles_names = GS_deciles_names9,
  row.names = NULL
) %>%
  mutate(GS_decile_min_rain = lag(GS_decile_max_rain9 + 0.1)) %>%
  mutate(defined_date = paste0("from ", date9, "to 14-10")) %>%
  select(GS_deciles_names, GS_decile_min_rain, GS_decile_max_rain, defined_date)
#add in the min value to table
GS_decile_table9[1,2] <-  min(gs_rain8$sum_rain_season)
